...
function klaro_breadcrumbs() {
	...
	if ( is_single() || is_page() ) {
		...
		echo '<span property="name" aria-current="page">' . sprintf( esc_html__( 'Search Results for: %s', 'klaro' ), esc_html( get_search_query() ) ) . '</span>';
		echo '<meta property="position" content="' . esc_attr( $position ) . '">';
		echo '</li>';
	} elseif ( is_404() ) {
		echo '<li property="itemListElement" typeof="ListItem">';
					...
					<div class="comment-metadata">
						<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
							<time datetime="<?php comment_time( 'c' ); ?>" itemprop="datePublished">
								...
								printf( esc_html__( '%1$s at %2$s', 'klaro' ), esc_html( get_comment_date( '', $comment ) ), esc_html( get_comment_time() ) );
								?>
							</time>
						</a>
						<?php
